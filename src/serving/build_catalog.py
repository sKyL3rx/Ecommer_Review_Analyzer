...
def main() -> None:
    ...
    agg_reviews = (
        reviews_df.groupby("product_id", as_index=False)
        .agg(
    ...
    catalog_df["rating_number"] = pd.to_numeric(catalog_df["rating_number"], errors="coerce").fillna(0).astype(int)
    ...
    catalog_df["search_text"] = (
        ...
        + catalog_df["categories_list"].apply(lambda xs: " ".join(xs) if isinstance(xs, list) else "") + " "
